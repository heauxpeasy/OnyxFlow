...
// Filterable fields: story_id, content, rating
async function fetchCommentEntities() {
    const response = await fetch(`/api/apps/69bf1ff0347c8d582a4daf61/entities/Comment`, {
        headers: {
            'api_key': 'ef0d0fbf0b38464e8ab4c70110248774', // or use await User.me() to get the API key
            'Content-Type': 'application/json'
        }
...
async function updateCommentEntity(entityId, updateData) {
    const response = await fetch(`/api/apps/69bf1ff0347c8d582a4daf61/entities/Comment/${entityId}`, {
        method: 'PUT',
        headers: {
            'api_key': 'ef0d0fbf0b38464e8ab4c70110248774', // or use await User.me() to get the API key
            'Content-Type': 'application/json'
